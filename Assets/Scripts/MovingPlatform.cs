...
public class MovingPlatform : MonoBehaviour
{
	...
	{
		var canStand = collision.collider.GetComponent<ICanStandOnMovingPlatform>();
		if (canStand == null) return;
		foreach (var contact in collision.contacts)
		{
			if (Vector2.Dot(Vector2.down, contact.normal) > 0.8f)
			{
				collision.collider.transform.SetParent(this.transform);
				canStand.EnterPlatform();
				break;
			}
		}
	}
